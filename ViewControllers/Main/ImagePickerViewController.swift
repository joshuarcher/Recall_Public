...
//  Reecall
//
//  Created by Joshua Archer on 3/12/16.
...
class ImagePickerViewController: UIViewController {
    ...
    private let datePickerSegue = "presentDatePicker"
...
//        showNextView()
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == datePickerSegue {
            ...
            guard let nextVc = segue.destinationViewController as? DatePickerViewController else {return}
...
extension ImagePickerViewController {
    ...
    func runSession() {
        ...
        dispatch_async(self.sessionQueue, {
            ...
            NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(ImagePickerViewController.subjectAreaDidChange(_:)), name: AVCaptureDeviceSubjectAreaDidChangeNotification, object: self.videoDeviceInput?.device)
            ...
            self.runtimeErrorHandlingObserver = NSNotificationCenter.defaultCenter().addObserverForName(AVCaptureSessionRuntimeErrorNotification, object: self.session, queue: nil, usingBlock: { (note: NSNotification?) -> Void in
                let strongSelf: ImagePickerViewController = weakSelf!
...
extension ImagePickerViewController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UICollectionViewDelegate {
